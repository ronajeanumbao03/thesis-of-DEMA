...
class DepartmentController extends Controller
{
    ...
    public function storeHead(Request $request)
    {
        ...
        $request->validate([
            ...
            'department_id' => 'required|exists:departments,id', // Make sure the department exists
        ]);

        // Find the department and assign the head
        $department = Department::findOrFail($request->department_id);
    ...
    public function deleteBudget($departmentId)
    {
        $department = Department::findOrFail($departmentId);

        // Optional: Clear budget fields if you want to preserve the department
        $department->annual_budget = null;
        $department->save();

        // Optionally delete associated budget record too
        Budget::where('department_id', $department->id)->delete();

        return redirect()->route('departments.index')->with('success', 'Budget deleted successfully.');
    }
